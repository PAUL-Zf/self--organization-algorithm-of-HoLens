...
def Self_Organization(start_time, end_time, entropy_threshold, dateType='Weekdays'):

    def matching_point_to_region(df, lng, lat, ST_tree):
        """
        Assign each point to the segmented area
        """
        blocks = {}
    ...
    # Read map data and track data
    area = gpd.read_file('./data/area.shp')
    ...
    neighbors = pk.load(open('./data/large_nyc_neighbors.pk', 'rb'))
    df_filter = pd.read_csv('./data/Large_NYC_high_order.csv')
    ...
    # Select the appropriate data based on the date type
    if(dateType == 'Weekdays'):
        df_od_duration = pd.read_csv(
            './data/df_od_duration_large_nyc_weekdays.csv')
        print('Here is weekdays!')
    else:
        df_od_duration = pd.read_csv(
            './data/df_od_duration_large_nyc_holidays.csv')
    ...
    merged_area.drop(['centroid', 'entropy', 'index'], axis=1).to_file(
        './data/merged_area.geojson', driver="GeoJSON")
...
if __name__ == '__main__':
    ...
    # to csv
    pathname1 = './data/merged_df_od_duration_' + \
        str(entropy_threshold) + '.csv'
    pathname2 = './data/merged_area_' + str(entropy_threshold) + '.csv'
